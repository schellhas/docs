...
- The selection problem can be solved such that the threads are arleady totally ordered with regard to the execution order.
- The first thread (``t_next``) in this sequence is selected.
...
- Besides the instruction pointer the processor keeps much more thread specific information in its registers:
	- Contents of arithmentic registers, index registers, processor state etc., that represent the **state of the execution** of the program, i.e. the thread.
	- Contents of address registers, segment tables, interrupt masks, access control information etc. that make up the threads execution environment.
...
- Context switch is the most time consuming part of a thread switch.
- To speed it up, the processors hardware can give some support:
	- By special instructions that allow storing the complete set of registers to memory in one instruction (and also restoring).
	- By providing several sets of registers (e.g. 8) on the processor, such that at thread switch only the register needs to be saved that indicates the number of the current valid register sets.
...
This sequence can be inserted in the thread's program at all places where a thread switch should take place.

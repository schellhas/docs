...
.. epigraph::
	...
	-- Antoine de Saint-Exupérty, 1900-1944
...
Should be in a tree-like hierarchy (of modules) so the system stays scalable and managed complexity well.

Layering
~~~~~~~~

Another approach. Each layer uses services from lower layers, uses them in their layers, and serves a higher more complex service for the layers above.

Example for Layering: Internet Protocol Stack
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Internet Protocol Stack uses layering.

End-to-End Principle (E2E) (Internet)
"""""""""""""""""""""""""""""""""""""

The elements in the same layer can communicate to each other and exchange information. They do this by giving this information to lower layers than forward the data, at the lowest layer in physical form.

Hourglass-Architecture (Internet)
"""""""""""""""""""""""""""""""""

A lot of different applications and services can use the Internet Protocol standard and signal to a lot of different networks, media and signal representations.

Hourglass-Architecture in OS
""""""""""""""""""""""""""""

Various applications use the OS programming interface (e.g. POSIX) to communicate to various different hardware.

E2E in the OS context: application neutrality
"""""""""""""""""""""""""""""""""""""""""""""

Customization should be as far "up" the hierarchy as possible, the lower, the more uniform it should be. (?)
...
In order to serve a wide generality of software with the OS or a component of the OS, we need to seperate the mechanism that implements this component and the policy and rules/management of this component. I didn't understand quite why tho.

Orthogonality
~~~~~~~~~~~~~

Functions and concepts of an OS should be independent of each other. They should operate in different *dimensions*. This way we can exchange components without interrupting the rest of the system.

SPOT-Rule (Single Point of Truth)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

We only want to hold our data in a single place, we don't want to have copies as we don't want to synchronize because this has a threat of diverging data in different places.

For code: Each functionality is implemented exactly once, we don't want duplicate code. Risk of diverging functionality if we change one

For data: Each piece of information can have only exactly one representation, for the same reasons.

Usage of SPOT avoids inconsistencies.

	*Architecture - Part 2 done*
